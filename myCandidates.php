<table id='mainTableBox' cellspacing="5px">
    ...
    $mc1->execute(array($office));
    $mc1row = $mc1->fetch();
    $cols = ($mc1row[0] >= 2) ? $mc1row[0] : 1;
    $colWidth = (100 / $cols) . "%";
    $fontSize = ($cols >= 4) ? "font-size:.75em;": "font-size:1em;";
    ?>
    <tr><td colspan='<?php echo $cols; ?>' style="border:1px solid <?php echo $highlightColor; ?>;"><div style="text-align:center; font-size:3em; font-weight:bold;">My 2016 Candidates</div></td></tr>
    <tr><td colspan='<?php echo $cols; ?>' style="border:1px solid <?php echo $highlightColor; ?>;"><div style="text-align:center; font-size:2em; font-weight:bold;"><?php echo $office; ?></div></td></tr>
    <tr><td colspan='<?php echo $cols; ?>'><div style="height:20px;">&nbsp;</div></td></tr>
    <?php
    // Reference by name
    $c = 1;
    $mc2 = $db->prepare("SELECT candName FROM candidates WHERE office = ? ORDER BY RAND()");
    $mc2->execute(array($office));
    ...
    echo "<tr>\n";
    for ($i = 1; $i <= $cols; $i++) {
        $mc = $db->prepare("SELECT picName, picExt FROM candidates WHERE candName = ?");
        $mc->execute(array(${"cand" . $i}));
        $mcrow = $mc->fetch();
    ...
    $mc3->execute(array($cand1));
    $mc3row = $mc3->fetch();
    $questions = explode(",", $mc3row['questions']);

    foreach ($questions as $k => $v) {
        $mc4 = $db->prepare("SELECT question FROM candidateQuestions WHERE id = ?");
        $mc4->execute(array($v));
        $mc4row = $mc4->fetch();
        $q = str_split($office);
        ...
        echo "<tr>\n";
        for ($i = 1; $i <= $cols; $i++) {
            $mc5 = $db->prepare("SELECT answer$v FROM candidates WHERE candName = ?");
            $mc5->execute(array(${"cand" . $i}));
            $mc5row = $mc5->fetch();
            $answer = nl2br(make_links_clickable(html_entity_decode($mc5row[0], ENT_QUOTES), $highlightColor));
