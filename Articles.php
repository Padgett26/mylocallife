<?php
$category = (null !== $_GET["category"]) ? htmlspecialchars(
        trim($_GET["category"])) : 0;

$artId = (filter_input(INPUT_GET, 'artId', FILTER_SANITIZE_NUMBER_INT) >= 1) ? filter_input(
        INPUT_GET, 'artId', FILTER_SANITIZE_NUMBER_INT) : False;
...
if ($artId) {
    ...
    if (filter_input(INPUT_GET, 'reportId', FILTER_SANITIZE_NUMBER_INT)) {
        if ($myId != '0') {
            ...
            $repUp = $db->prepare(
                    "INSERT INTO reported VALUES" . "(NULL,?,?,?,?,?,?,'0','0')");
            $repUp->execute(
                    array(
                            $myId,
                            $artId,
    ...
    } elseif (filter_input(INPUT_GET, 'reportArticle',
            FILTER_SANITIZE_NUMBER_INT)) {
        $repArt = filter_input(INPUT_GET, 'reportArticle',
                FILTER_SANITIZE_NUMBER_INT);
        ...
        $repstmt = $db->prepare(
                "SELECT COUNT(*) FROM reported WHERE articleId=? && reportedTime >= 1 && clearedTime = 0");
...
<?php
            ...
            if ($myId != '0') {
                $repstmt2 = $db->prepare(
                        "SELECT COUNT(*) FROM reported WHERE articleId=? && reportedBy=?");
