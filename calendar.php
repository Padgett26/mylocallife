...
if ($myId != '0') {
    if (filter_input(INPUT_POST, 'title', FILTER_SANITIZE_STRING)) {
        ...
        $stmt = $db->prepare(
                "INSERT INTO calendar VALUES(NULL,?,?,?,'0.jpg','0','0',?,'0','0','0')");
        $stmt->execute(array(
                $upStartTime,
                $upTitle,
                $upWriteUp,
        ...
        $getId = $db->prepare(
                "SELECT id FROM calendar WHERE startTime=? && title=? && writeUp=? && userId=? ORDER BY id DESC LIMIT 1");
        $getId->execute(array(
                $upStartTime,
                $upTitle,
                $upWriteUp,
...
$_SESSION['preview'] = (filter_input(INPUT_GET, 'preview',
        FILTER_SANITIZE_NUMBER_INT)) ? filter_input(INPUT_GET, 'preview',
        FILTER_SANITIZE_NUMBER_INT) : 3;
$p = $_SESSION['preview'];
$h = (filter_input(INPUT_GET, 'h', FILTER_SANITIZE_NUMBER_INT)) ? filter_input(
        INPUT_GET, 'h', FILTER_SANITIZE_NUMBER_INT) : 0;
$m = date('n') + $p;
$y = date('Y');
...
$stmt = $db->prepare(
        "SELECT * FROM calendar WHERE startTime >= ? && startTime <= ? && approved = '1' ORDER BY startTime");
$stmt->execute(array(
        $lowerLimit,
        $limit
));
while ($row = $stmt->fetch()) {
    ...
    echo "<a href='index.php?page=calendar&h=" . $cId . "#c" . $cId .
            "'><div id='c$cId'></div><div style='text-align:center; width:98%; border:2px solid $highlightColor; cursor:pointer; padding:5px;' ><span style='font-size:1.5em; font-weight:bold; text-decoration:none;'>$cTitle</span><br />";
    echo "<div style='font-size:1em; text-align:center;'>" .
            date('M jS, g:ia', $cStartTime) . "</div></div></a>";
    $dis = ($h == $cId) ? "block" : "none";
    if (file_exists("userPics/$cUserId/$cPic")) {
        list ($wi, $hi) = (getimagesize("userPics/$cUserId/$cPic") != null) ? getimagesize(
                "userPics/$cUserId/$cPic") : null;
        $maxW = 300;
        $maxH = 400;
        ...
        echo "<div id='e$cId' style='margin:0px 20px; border:1px solid $highlightColor; display:$dis; padding:10px; min-height:" .
                $th . "px;'>";
        echo "<a href='userPics/$cUserId/$cPic' data-lightbox='images'><img src='userPics/$cUserId/$cPic' style='float:right; margin:0px 0px 5px 5px; padding:3px; border:1px solid $highlightColor; max-width:" .
                $maxW . "px; max-height:" . $maxH . "px;' /></a>";
        echo "";
    } else {
