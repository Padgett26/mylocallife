...
$classifiedTitle = trim ( filter_input ( INPUT_POST, 'classifiedTitle', FILTER_SANITIZE_STRING ) );
$a2 = htmlEntities ( trim ( $_POST ['classifiedText'] ), ENT_QUOTES );
$classifiedText = filter_var ( $a2, FILTER_SANITIZE_STRING );
$activateClassified = filter_input ( INPUT_POST, 'activateClassified', FILTER_SANITIZE_NUMBER_INT );
$goodUntil = ($activateClassified == "1") ? ($time + 1209600) : '0'; // advance goodUntil two weeks
$catId = filter_input ( INPUT_POST, 'catId', FILTER_SANITIZE_NUMBER_INT );
$delClassified = (filter_input ( INPUT_POST, 'delClassified', FILTER_SANITIZE_NUMBER_INT ) == '1') ? "1" : "0";
...
if ($delClassified == "1") {
	...
	$getchr = $db->prepare ( "SELECT classifiedTextLength FROM classifieds WHERE id=?" );
	...
	$clsstmt = $db->prepare ( "UPDATE classifieds SET classifiedTitle=?, classifiedText=?, displayUntil=?, catId=? WHERE id=?" );
	$clsstmt->execute ( array (
			$classifiedTitle,
			$t,
			$goodUntil,
			$catId,
	...
	if ($chrlen >= 251) {
		...
		if ($width != null && $height != null) {
			...
			processPic ( $myId, $imageName, '250', '250', $image, $imageType );
			processThumbPic ( $myId, $imageName, '100', '100', $image, $imageType );
			$p1stmt = $db->prepare ( "UPDATE classifieds SET picName=?, picExt=? WHERE id=?" );
