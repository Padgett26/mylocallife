...
$delStrip = (filter_input(INPUT_POST, 'delStrip', FILTER_SANITIZE_NUMBER_INT) ==
        '1') ? "1" : "0";
$removeBackPic = (filter_input(INPUT_POST, 'removeBackPic',
...
if ($delStrip == "1") {
    ...
    if ($stripId == "new") {
        ...
        $getidstmt = $db->prepare(
                "SELECT id FROM strips WHERE stripTitle=? && userId=? ORDER BY id DESC LIMIT 1");
    ...
    list ($widthBack, $heightBack) = (getimagesize($imageBack) != null) ? getimagesize(
            $imageBack) : null;
    ...
    $stripstmt = $db->prepare(
            "UPDATE strips SET stripTitle=?, displayDayStart=?, displayDayEnd=? WHERE id=?");
    $stripstmt->execute(
            array(
                    $stripTitle,
                    $displayDayStart,
