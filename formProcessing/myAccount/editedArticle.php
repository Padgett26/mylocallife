...
$articleTitle = trim ( filter_input ( INPUT_POST, 'articleTitle', FILTER_SANITIZE_STRING ) );
...
$pic1Caption = trim ( filter_input ( INPUT_POST, 'pic1Caption', FILTER_SANITIZE_STRING ) );
$pic2Caption = trim ( filter_input ( INPUT_POST, 'pic2Caption', FILTER_SANITIZE_STRING ) );
...
$yt = filter_input ( INPUT_POST, 'youtube', FILTER_SANITIZE_URL ) ? trim ( filter_input ( INPUT_POST, 'youtube', FILTER_SANITIZE_URL ) ) : '///0';
$youtube = explode ( "/", $yt );
...
$delPdf1 = (filter_input ( INPUT_POST, 'delPdf1', FILTER_SANITIZE_NUMBER_INT ) == '1') ? "1" : "0";
$delPdf2 = (filter_input ( INPUT_POST, 'delPdf2', FILTER_SANITIZE_NUMBER_INT ) == '1') ? "1" : "0";
$delArticle = (filter_input ( INPUT_POST, 'delArticle', FILTER_SANITIZE_NUMBER_INT ) == '1') ? "1" : "0";

if ($delArticle == "1") {
	$stmt = $db->prepare ( "SELECT pic1Name, pic1Ext, pic2Name, pic2Ext, pdf1, pdf2 FROM articles WHERE id=?" );
	$stmt->execute ( array (
			$artId
	) );
	$row = $stmt->fetch ();
	...
	if ($artId == "new") {
		$n = 1;
		$newartstmt = $db->prepare ( "INSERT INTO articles VALUES" . "(NULL, 'title', 'text', '0', 'jpg', NULL, '0', 'jpg', NULL, ?, 'local', ?, '0', NULL, '0', '0', '0', NULL, '0', NULL, '0', '', '0', '0', '0', '0')" );
		$newartstmt->execute ( array (
				$postedDate,
				$myId
		) );
		$getidstmt = $db->prepare ( "SELECT id FROM articles WHERE postedDate=? && authorId=? ORDER BY id DESC LIMIT 1" );
	...
	if (isset ( $_FILES ['newPdf1'] ['name'] ) && $delPdf1 != "1") {
		processPdf ( $myId, $time, '1', $_FILES ['newPdf1'] ['tmp_name'], $artId, $db );
	}
	if ($delPdf1 == '1') {
		deletePdf ( $myId, '1', $artId, $db );
	}
	if (isset ( $_FILES ['newPdf2'] ['name'] ) && $delPdf2 != "1") {
		processPdf ( $myId, $time, '2', $_FILES ['newPdf2'] ['tmp_name'], $artId, $db );
	}
	if ($delPdf2 == '1') {
		deletePdf ( $myId, '2', $artId, $db );
	}

	$isedited = $db->prepare ( "SELECT COUNT(*) FROM reported WHERE reportedBy = '0' && reportedTime >= '1' && clearedTime = '0'" );
	$isedited->execute ();
	$isrow = $isedited->fetch ();
	$editedDate = ($n != 1) ? $time : "0";
	$artstmt = $db->prepare ( "UPDATE articles SET articleTitle=?, articleText=?, pic1Caption=?, pic2Caption=?, editedDate=?, catId=?, inReplyTo=?, youtube=?, pdfText1=?, pdfText2=?, embedCode1=?, slug=? WHERE id=?" );
