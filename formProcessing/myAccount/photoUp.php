...
$delPhoto = (filter_input(INPUT_POST, 'delPhoto', FILTER_SANITIZE_NUMBER_INT) ==
        '1') ? '1' : '0';
...
if ($delPhoto == '1') {
    ...
    $p5 = $db->prepare(
            "SELECT photoName, photoExt FROM photoList WHERE photoId=?");
    $p5->execute(array(
            $photoId
    ));
    while ($pr5 = $p5->fetch()) {
        if (file_exists(
                "userPics/$myId/" . $pr5['photoName'] . "." . $pr5['photoExt'])) {
            unlink(
                    "userPics/$myId/" . $pr5['photoName'] . "." .
                    $pr5['photoExt']);
        }
        if (file_exists(
                "userPics/$myId/thumb/" . $pr5['photoName'] . "." .
                $pr5['photoExt'])) {
            unlink(
                    "userPics/$myId/thumb/" . $pr5['photoName'] . "." .
                    $pr5['photoExt']);
    ...
    $p3 = $db->prepare(
            "UPDATE photoJournalism SET photoTitle=?, photoText=? WHERE id=?");
    $p3->execute(array(
            $photoTitle,
            $photoText,
            $photoId
    ...
    foreach ($_FILES as $key => $val) {
        if (preg_match("/^image([1-9][0-9]*)$/", $key, $match)) {
            ...
            if ($width != null && $height != null) {
                ...
                if ($iId < $pCount) {
                    $pstmt3 = $db->prepare(
                            "UPDATE photoList SET photoName = ?, photoExt = ? WHERE photoId = ? && photoOrder = ?");
                    $pstmt3->execute(
                            array(
                                    $imageName,
                                    $imageType,
                    ...
                    $pstmt2 = $db->prepare(
                            "INSERT INTO photoList VALUES(NULL,?,?,?,?,?,'0','0','0')");
                    $pstmt2->execute(
                            array(
                                    $photoId,
                                    $imageName,
    ...
    foreach ($_POST as $key => $val) {
        if (preg_match("/^photoCaption([1-9][0-9]*)$/", $key, $match)) {
            ...
            $pstmt = $db->prepare(
                    "UPDATE photoList SET photoCaption=? WHERE photoId=? && photoOrder=?");
            $pstmt->execute(array(
                    $photoCaption,
                    $photoId,
                    $cId
        ...
        if (preg_match("/^photoOrder([1-9][0-9]*)$/", $key, $match)) {
            ...
            $p10 = $db->prepare(
                    "SELECT id FROM photoList WHERE photoId = ? && photoOrder = ?");
            $p10->execute(array(
                    $photoId,
                    $oId
            ));
            $pr10 = $p10->fetch();
            $oldOrderId = $pr10['id'];
            // Move all of the pictures who are equal to or later in the order
            // by +1
            $p7 = $db->prepare(
                    "UPDATE photoList SET photoOrder = photoOrder + 1 WHERE photoOrder >= ? && photoId = ?");
            $p7->execute(array(
                    $photoOrder,
                    $photoId
            ));
            ...
            $p11 = $db->prepare(
                    "UPDATE photoList SET photoOrder = ? WHERE id = ?");
            $p11->execute(array(
                    $photoOrder,
                    $oldOrderId
            ));
            ...
            $t = 1;
            $p12 = $db->prepare(
                    "SELECT id FROM photoList WHERE photoId = ? ORDER BY photoOrder");
            $p12->execute(array(
                    $photoId
            ));
