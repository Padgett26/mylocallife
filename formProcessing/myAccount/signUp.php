...
$title = htmlentities(
        filter_input(INPUT_POST, 'title', FILTER_SANITIZE_STRING), ENT_QUOTES);
$description = htmlentities(
        filter_input(INPUT_POST, 'description', FILTER_SANITIZE_STRING),
        ENT_QUOTES);
$startSignUp = filter_input(INPUT_POST, 'startSignUp',
...
if ($title != "" && $description != "") {
    ...
    $u1 = $db->prepare(
            "INSERT INTO signUp VALUES(NULL,?,?,?,'x.jpg',?,?,'0','0','0')");
    $u1->execute(array(
            $myId,
            $title,
            $description,
    ...
    $u2 = $db->prepare(
            "SELECT id FROM signUp WHERE userId = ? AND title = ? AND description = ? AND startSignUp = ? AND endSignUp = ? ORDER BY id DESC LIMIT 1");
    $u2->execute(array(
            $myId,
            $title,
            $description,
    ...
    for ($i = 1; $i <= 3; ++ $i) {
        if (${"useTS" . $i} == 1) {
            ...
            $st = mktime(${"startHour" . $i}, ${"startMin" . $i}, 0, $sd[1], $sd[2],
                    $sd[0]);
            $ed = explode("-", ${"endDate" . $i});
            $et = mktime(${"endHour" . $i}, ${"endMin" . $i}, 0, $ed[1], $ed[2],
                    $ed[0]);
            if (${"subMin" . $i} >= 1) {
                $t = (${"subMin" . $i} * 60);
                for ($j = $st; $j < $et; $j = $j + $t) {
                    $u3 = $db->prepare(
                            "INSERT INTO signUpTimes VALUES(NULL,?,?,?,?,'0','0','0')");
                    $u3->execute(
                            array(
                                    $suId,
                                    $j,
