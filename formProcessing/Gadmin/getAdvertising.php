...
$linkLocal = (filter_input ( INPUT_POST, 'linkLocal', FILTER_SANITIZE_NUMBER_INT ) == '0') ? '0' : '1';
...
if ($delAd == '1') {
	...
	if ($adId == 'new') {
		$ad1 = $db->prepare ( "INSERT INTO advertising VALUES(NULL, ?, ?, ?, '0', 'jpg', ?, ?, ?, '0','0','0')" );
		$ad1->execute ( array (
				$adUserId,
				$slot,
				$time,
		...
		$ad2 = $db->prepare ( "SELECT id FROM advertising WHERE userId=? && activeUntil=? ORDER BY id DESC LIMIT 1" );
		$ad2->execute ( array (
				$adUserId,
				$time
		) );
		...
		$ad3 = $db->prepare ( "UPDATE advertising SET linkLocal=?, linkText=?, salesRepId=? WHERE id=?" );
		$ad3->execute ( array (
				$linkLocal,
				$linkText,
				$salesRepId,
	...
	list ( $width, $height ) = (getimagesize ( $adImage ) != null) ? getimagesize ( $adImage ) : null;
	if ($width != null && $height != null) {
		...
		processPic ( $adUserId, $time, $x, $y, $adImage, $imageType );
		$astmt = $db->prepare ( "UPDATE advertising SET adName=?, adExt=? WHERE id=?" );
		$astmt->execute ( array (
				$time,
				$imageType,
				$adId
