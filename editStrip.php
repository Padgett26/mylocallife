...
$stmt->execute(array($stripId, $myId));
$row = $stmt->fetch();
...
if ($userId == $myId || ($myId != '0' && $stripId == "new")) {
    ...
    <div id='mainTableBox' style="padding:10px;">
        <form action="index.php?page=myAccount" method="post" enctype='multipart/form-data'>
            ...
            <input type="text" name="stripTitle" value="<?php echo $stripTitle; ?>" size="70" max-length="100" /><br /><br />
            <?php
            ...
            <?php } ?>
            <input type='hidden' name='imageWidth' value='800' /><input type='hidden' name='imageHeight' value='800' />
            <input type="hidden" name="editedStrip" value="<?php echo $stripId; ?>" /><input type="submit" value=" Save changes " />
        </form>
    </div>
    <?php
} else {
