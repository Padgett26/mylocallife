...
if ($myId != '0') {
    ...
    if ($myAccess != '0') {
        ...
        if (filter_input(INPUT_POST, 'blogDescriptions',
                FILTER_SANITIZE_NUMBER_INT) == $myId) {
            include "formProcessing/myAccount/blogDescriptions.php";
        }

        // Delete entire blogs
        if (filter_input(INPUT_POST, 'blogDel', FILTER_SANITIZE_NUMBER_INT) ==
                '1') {
        ...
        if (filter_input(INPUT_POST, 'signUpDel', FILTER_SANITIZE_NUMBER_INT) >=
                1) {
            include "formProcessing/myAccount/signUp.php";
        }
        if (filter_input(INPUT_POST, 'signUpNew', FILTER_SANITIZE_NUMBER_INT) ==
                1) {
            include "formProcessing/myAccount/signUp.php";
        }
    }
...
<main id='mainTableBox'
        ...
        if ($row4['businessListing'] == '0') {
            ...
            $busiDescText = nl2br(
                    make_links_clickable(
                            html_entity_decode($row5['busiDescText'], ENT_QUOTES)));
					...
					<tr>
						...
						<td style="border: 1px solid #aaaaaa; padding: 10px;"><select
							name="category" size='1'>
                                        <?php
            $getC = $db->prepare(
                    "SELECT * FROM busiCategories ORDER BY busiCatName");
            $getC->execute();
	...
	<section>
        ...
        $stmt = $db->prepare(
                "SELECT DISTINCT catId FROM articles WHERE authorId=?");
        $stmt->execute(array(
                $myId
        ));
        while ($row = $stmt->fetch()) {
            ...
            $t = 0;
            $substmt2 = $db->prepare(
                    "SELECT * FROM articles WHERE authorId=? && catId=? ORDER BY postedDate DESC");
            ...
            while ($row = $substmt2->fetch()) {
                ...
                if ($embedCode1 != "" && $embedCode1 != " ") {
                    $articleText = str_replace("<*embedCode1*>", $embedCode1,
                            $articleText);
                } else {
                    $articleText = str_replace("<*embedCode1*>", "",
                            $articleText);
                }
                    ...
                    if ($textLen >= 1000) {
                        ...
                        if ($pos2 > $pos1) {
                            echo substr($articleText, $pos1 + 1,
                                    $pos2 - $pos1 - 1);
                            if (file_exists(
                                    "userPics/$authorId/$pic2Name.$pic2Ext")) {
	...
	<section>
        ...
        $wr1 = $db->prepare(
                "SELECT DISTINCT bookId FROM myWritings WHERE authorId = ? ORDER BY title");
        $wr1->execute(array(
                $myId
        ));
        while ($wr1R = $wr1->fetch()) {
            $wrId = $wr1R['bookId'];
            $wr2 = $db->prepare(
                    "SELECT title, approved FROM myWritings WHERE authorId = ? AND bookId = ? LIMIT 1");
            $wr2->execute(array(
                    $myId,
                    $wrId
            ));
	...
	<section>
        ...
        $t = 0;
        $pst1 = $db->prepare(
                "SELECT * FROM photoJournalism WHERE authorId=? ORDER BY postedDate DESC");
        $pst1->execute(array(
                $myId
        ));
        while ($prow1 = $pst1->fetch()) {
            ...
            $photoText = nl2br(
                    make_links_clickable(
                            html_entity_decode($prow1['photoText'], ENT_QUOTES)));
            ...
            while ($prow2 = $pst2->fetch()) {
                ...
                $photoCaption = nl2br(
                        make_links_clickable($prow2['photoCaption']));
	...
	<section>
        ...
        $stmt = $db->prepare(
                "SELECT DISTINCT catId FROM classifieds WHERE userId=?");
        $stmt->execute(array(
                $myId
        ));
        while ($row = $stmt->fetch()) {
            ...
            $t = 0;
            $substmt2 = $db->prepare(
                    "SELECT * FROM classifieds WHERE userId=? && catId=? ORDER BY displayUntil");
            $substmt2->execute(array(
                    $myId,
                    $catId
            ));
            while ($subrow2 = $substmt2->fetch()) {
                ...
                $classifiedText = nl2br(
                        make_links_clickable(
                                html_entity_decode($subrow2['classifiedText'],
                                        ENT_QUOTES)));
                ...
                if ($displayUntil > 1) {
                    echo "Display until: " . date("M j, Y", $displayUntil) .
                            "<br /><br />";
                }
	...
	<section>
        ...
        $dateY = $db->prepare(
                "SELECT DISTINCT postedYear FROM blog WHERE userId=? ORDER BY postedDate");
        $dateY->execute(array(
                $myId
        ));
        while ($dateYrow = $dateY->fetch()) {
            ...
            $dateM = $db->prepare(
                    "SELECT DISTINCT postedMonth FROM blog WHERE userId=? && postedYear=? ORDER BY postedDate");
            $dateM->execute(array(
                    $myId,
                    $getY
            ));
            while ($dateMrow = $dateM->fetch()) {
                ...
                echo "<a href='index.php?page=editBlog&blogUserId=$myId&showM=$getM&showY=$getY'>Edit blog entries from " .
                        $months[$getM] . " " . $getY . "</a>";
                echo "</div>";
            }
        }
	...
	<section>
		<header onclick="toggleview('accMyAds')"
            ...
            <?php
        $showAds = (filter_input(INPUT_GET, 'showAds',
                FILTER_SANITIZE_NUMBER_INT) == '1') ? "block" : "none";
        ?>
        ...
        while ($rowa = $stmta->fetch()) {
            ...
            if (file_exists("userPics/$myId/" . $adName . "." . $adExt)) {
                echo "<img src='userPics/$myId/" . $adName . "." . $adExt .
	...
	<section>
        ...
        $stmt = $db->prepare(
                "SELECT t1.id, t1.question FROM surveyQuestions AS t1 INNER JOIN survey AS t2 ON t1.surveyId = t2.id WHERE t2.userId = ? ORDER BY t2.startDate DESC");
        $stmt->execute(array(
                $myId
        ));
        while ($r = $stmt->fetch()) {
	...
	<section>
        ...
        $stmt3 = $db->prepare(
                "SELECT DISTINCT stripTitle FROM strips WHERE userId=?");
        $stmt3->execute(array(
                $myId
        ));
        while ($row3 = $stmt3->fetch()) {
            ...
            $t = 0;
            $stmt4 = $db->prepare(
                    "SELECT * FROM strips WHERE userId=? && stripTitle=? ORDER BY displayDayStart DESC");
            $stmt4->execute(array(
                    $myId,
                    $stripTitle
            ));
            while ($row4 = $stmt4->fetch()) {
                ...
                if (date("M j, Y", $displayDayStart) ==
                        date("M j, Y", $displayDayEnd)) {
	...
	<section>
        ...
        echo "<tr><td id='signuptd'>Open sign up:</td><td id='signuptd'><input type='date' name='startSignUp' value='" .
                date("Y-m-d", $time) . "'></td></tr>";
        echo "<tr><td id='signuptd'>End sign up:</td><td id='signuptd'><input type='date' name='endSignUp' value='" .
                date("Y-m-d", ($time + 604800)) . "'></td></tr>";
        for ($i = 1; $i <= 3; ++ $i) {
            ...
            echo "</td></tr>";
            echo "<tr><td id='signuptd'>Start date and time</td><td id='signuptd'><input type='date' name='startDate$i' value='" .
                    date("Y-m-d", ($time + 604800)) .
                    "'> <input type='number' name='startHour$i' value='12' step='1' min='0' max='23'> : <input type='number' name='startMin$i' value='0' step='1' min='0' max='59'></td></tr>";
            echo "<tr><td id='signuptd'>End date and time</td><td id='signuptd'><input type='date' name='endDate$i' value='" .
                    date("Y-m-d", ($time + 604800)) .
                    "'> <input type='number' name='endHour$i' value='12' step='1' min='0' max='23'> : <input type='number' name='endMin$i' value='0' step='1' min='0' max='59'></td></tr>";
        ...
        $g = $db->prepare(
                "SELECT * FROM signUp WHERE userId = ? ORDER BY endSignUp DESC");
        $g->execute(array(
                $myId
        ));
        while ($gR = $g->fetch()) {
            ...
            $description = nl2br(
                    html_entity_decode($gR['description'], ENT_QUOTES));
            $pic = $gR['pic'];
            ...
            echo "</td></tr>";
            echo "<tr><td id='signuptd'>Open sign up:</td><td id='signuptd'>" .
                    date("Y-m-d", $startSignUp) . "</td></tr>";
            echo "<tr><td id='signuptd'>End sign up:</td><td id='signuptd'>" .
                    date("Y-m-d", $endSignUp) . "</td></tr>";
            echo "<tr><td colspan='2' id='signuptd'>Event times:</td></tr>";
            $g2 = $db->prepare(
                    "SELECT * FROM signUpTimes WHERE subOfId = ? ORDER BY start");
            $g2->execute(array(
                    $id
            ));
            while ($g2R = $g2->fetch()) {
                ...
                $start = $g2R['start'];
                $end = $g2R['end'];
                echo "<tr><td id='signuptd'>Start: " . date("H:i", $start) .
                        "<br>End: " . date("H:i", $end) . "<br>" .
                        date("Y-m-d", $start) . "</td><td id='signuptd'>";
                $g3 = $db->prepare(
                        "SELECT * FROM signUpRegistry WHERE signUpId = ? ORDER BY registerTime");
                $g3->execute(array(
                        $tId
                ));
                while ($g3R = $g3->fetch()) {
                    ...
                    echo "<div style='display:none; padding:10px 20px;' id='reg$rId'>$email<br>$phone<br>Registered on: " .
                            date("Y-m-d H:i", $registerTime) . "</div>";
                }
                echo "</td></tr>";
            }
            echo "</table></div>";
