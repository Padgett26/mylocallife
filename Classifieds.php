...
if ($category) {
    ...
    if ($myZip != 0) {
        ...
        for ($i = 1; $i <= 3; $i ++) {
            ...
            $stmt6 = $db->prepare(
                    "SELECT t1.id, t1.classifiedTitle, t1.classifiedText FROM classifieds AS t1 INNER JOIN users AS t2 ON t1.userId = t2.id${"where" . $i} ORDER BY t1.displayUntil DESC");
            $stmt6->execute(array(
                    $catId,
                    $time
            ));
        ...
        echo "<table id='mainTableBox' cellspacing='5'><tr>";
        $stmt18 = $db->prepare(
                "SELECT id FROM classifieds WHERE catId = ? && displayUntil >= ? ORDER BY displayUntil DESC LIMIT 100");
    ...
    if ($category == "Help Wanted") {
        $x = 0;
        echo "<table id='mainTableBox' cellspacing='5'><tr>";
        $ccdcJobs = $db_ccdc->prepare(
                "SELECT title, content FROM jobs ORDER BY RAND()");
        $ccdcJobs->execute();
        while ($cJobs = $ccdcJobs->fetch()) {
            $cTitle = $cJobs['title'];
            $cContent = nl2br(
                    make_links_clickable(
                            html_entity_decode($cJobs['content'], ENT_QUOTES)));
