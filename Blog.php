...
if ($blogUserId && $blogUserId != "x") {
    $showM = (filter_input(INPUT_GET, 'showM', FILTER_SANITIZE_NUMBER_INT)) ? filter_input(
            INPUT_GET, 'showM', FILTER_SANITIZE_NUMBER_INT) : date("n");
    $showY = (filter_input(INPUT_GET, 'showY', FILTER_SANITIZE_NUMBER_INT)) ? filter_input(
            INPUT_GET, 'showY', FILTER_SANITIZE_NUMBER_INT) : date("Y");
    $check = $db->prepare(
            "SELECT COUNT(*) FROM blog WHERE userId=? && postedMonth=? && postedYear=?");
    $check->execute(array(
            $blogUserId,
            $showM,
            $showY
    ...
    if ($checkrow[0] == 0) {
        $check2 = $db->prepare(
                "SELECT postedMonth, postedYear FROM blog WHERE userId=? ORDER BY postedDate DESC LIMIT 1");
    ...
    $logstmt = $db->prepare(
            "INSERT INTO blogLog VALUES" . "(NULL,?,?,'0','0','0')");
    $logstmt->execute(array(
            $blogUserId,
            $time
    ));
    $logstmt2 = $db->prepare(
            "UPDATE blogDescriptions SET totalViews = totalViews + 1 WHERE userId = ?");
    ...
    $blogDesc = nl2br(
            make_links_clickable(
                    html_entity_decode($bdrow['blogDesc'], ENT_QUOTES)));
    ...
    while ($dateYrow = $dateY->fetch()) {
        ...
        while ($dateMrow = $dateM->fetch()) {
            ...
            if ($getM == $showM && $getY == $showY) {
                echo "<span style='font-weight:bold;'>" . $months[$getM] . " " .
                        $getY . "</span>";
            } else {
                echo "<a href='index.php?page=Blog&blogUserId=$blogUserId&showM=$getM&showY=$getY'>" .
                        $months[$getM] . " " . $getY . "</a>";
            }
            echo "</div>";
        }
    ...
    if ($myId != '0') {
        ...
        $t = 0;
        $getF = $db->prepare(
                "SELECT blogUserId FROM blogFavorites WHERE myId=? ORDER BY RAND()");
        $getF->execute(array(
                $myId
        ));
        while ($getFrow = $getF->fetch()) {
            ...
            $subF = $db->prepare(
                    "SELECT t1.blogTitle, t2.firstName, t2.lastName FROM blogDescriptions AS t1 INNER JOIN users AS t2 ON t1.userId = t2.id WHERE t2.id = ?");
            $subF->execute(array(
                    $buId
            ));
            $subFrow = $subF->fetch();
    ...
    if ($myId != '0') {
        $find = $db->prepare(
                "SELECT COUNT(*) FROM blogFavorites WHERE myId=? && blogUserId=?");
        $find->execute(array(
                $myId,
                $blogUserId
        ));
    ...
    ?>
  <div style='float:right; margin:3px 10px 0px 0px;' class="fb-share-button" data-href="https://mylocal.life/index.php?page=Blog&blogUserId=<?php

echo $blogUserId;
    ?>" data-layout="button" data-size="small"><a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=https%3A%2F%2Fmylocal.life%2Findex.php%3Fpage%3DBlog%26blogUserId%3D<?php

echo $blogUserId;
    ?>&amp;src=sdkpreparse" class="fb-xfbml-parse-ignore">Share</a></div>
  <div style='float:right; margin:3px 10px 0px 0px;'><a class="twitter-share-button" href="https://twitter.com/intent/tweet?original_referer=https%3A%2F%2Fmylocal.life%2Findex.php?page=Blog%26blogUserId=<?php

echo $blogUserId;
    ?>&ref_src=twsrc%5Etfw&related=twitterapi%2Ctwitter&tw_p=tweetbutton&url=https%3A%2F%2Fmylocal.life%2Findex.php?page=Blog%26blogUserId=<?php

echo $blogUserId;
    ?>&via=MyLocalLife" data-size="large">Tweet</a><script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script></div>
  <?php
    echo "<button onclick='print()' style='float:right; margin:0px 5px;'>Print</button>";
    echo "<div id='printArea'>";
    $blog = $db->prepare(
            "SELECT * FROM blog WHERE userId=? && postedMonth=? && postedYear=? ORDER BY postedDate");
    $blog->execute(array(
            $blogUserId,
            $showM,
            $showY
    ...
    while ($blogrow = $blog->fetch()) {
        ...
        $blogEntryText = nl2br(
                make_links_clickable(
                        html_entity_decode($blogrow['blogEntryText'], ENT_QUOTES)));
        $postedDate = $blogrow['postedDate'];
        $picName1 = $blogrow['picName1'];
        $picExt1 = $blogrow['picExt1'];
        $picCaption1 = make_links_clickable($blogrow['picCaption1']);
        $picName2 = $blogrow['picName2'];
        $picExt2 = $blogrow['picExt2'];
        $picCaption2 = make_links_clickable($blogrow['picCaption2']);
        $picName3 = $blogrow['picName3'];
        $picExt3 = $blogrow['picExt3'];
        $picCaption3 = make_links_clickable($blogrow['picCaption3']);
        $picName4 = $blogrow['picName4'];
        $picExt4 = $blogrow['picExt4'];
        $picCaption4 = make_links_clickable($blogrow['picCaption4']);

        echo "<div style='text-align:left; margin:10px 0px;'>Posted on: " .
                date("j M Y", $postedDate) . "</div>";
        echo "<header style='font-weight:bold; font-size:2em;'>$blogEntryTitle</header><br />";
        echo "<article style='text-align:justify;'>";
        $t = 0;
        for ($i = 1; $i <= 4; $i ++) {
            if (file_exists(
                    "userPics/$blogUserId/" . ${'picName' . $i} . "." .
                    ${'picExt' . $i})) {
                $t ++;
            }
        }
        if ($t != 0) {
            ...
            for ($i = 1; $i <= $t; $i ++) {
                ...
                echo ($i <= $t) ? substr($blogEntryText, $pos0, ($pos1 - $pos0)) : substr(
                        $blogEntryText, $pos0);
                $pos0 = $pos1;
                if (file_exists(
                        "userPics/$blogUserId/" . ${'picName' . $i} . "." .
                        ${'picExt' . $i})) {
                    echo "<a href='userPics/$blogUserId/" . ${'picName' . $i} . "." .
                            ${'picExt' . $i} .
                            "' data-lightbox='images'><img src='userPics/$blogUserId/" .
                            ${'picName' . $i} . "." . ${'picExt' . $i} .
                            "' alt='' style=' style='border:1px solid #aaaaaa; padding:10px; width:390px; float:";
        ...
        $b2 = $db->prepare(
                "SELECT COUNT(*) FROM blogVideos WHERE blogEntryId=?");
        $b2->execute(array(
                $blogEntryId
        ));
        $b2row = $b2->fetch();
        if ($b2row[0] >= 1) {
            $b3 = $db->prepare(
                    "SELECT videoTitle, videoAddress FROM blogVideos WHERE blogEntryId=? ORDER BY videoOrder");
            $b3->execute(array(
                    $blogEntryId
            ));
    ...
    $blogList = $db->prepare(
            "SELECT id FROM blogDescriptions ORDER BY RAND() LIMIT 39");
    $blogList->execute();
