...
if ($slug != "") {
    $cSplit = explode("::", $slug);
    if ($cSplit[0] == "Category") {
        $cat = $cSplit[1];
        $artSlug = 0;
    } elseif ($cSplit[0] == "reportArticle") {
        $repArt = $cSplit[1];
    } else {
        $artSlug = $cSplit[0];
    }
...
$artId = ($repArt >= 1) ? $repArt : 0;
if ($artSlug != 0) {
    $art1 = $db->prepare("SELECT id,slug FROM articles");
    $art1->execute();
    while ($art1r = $art1->fetch()) {
        $artId = ($art1r['slug'] == $artSlug) ? $art1r['id'] : $artId;
    }
}

$category = 0;
$cat1 = $db->prepare("SELECT id FROM articleCategories WHERE slug = ?");
$cat1->execute(array(
        $cat
));
$cat1r = $cat1->fetch();
...
if ($artId >= 1) {
    ...
    if (filter_input(INPUT_POST, 'reportId', FILTER_SANITIZE_NUMBER_INT)) {
        if ($myId != '0') {
            ...
            $repUp = $db->prepare(
                    "UPDATE articles SET approved = '2', whyReported = ?, whoReported = ? WHERE id = ?");
            $repUp->execute(array(
                    $whyReported,
                    $myId,
                    $artId
        ...
        if ($myId != '0' && $repArt == $artId) {
            ...
            $stmtrep = $db->prepare(
                    "SELECT articleTitle FROM articles WHERE id=?");
            ...
            echo "<form action='Articles/$artSlug' method='post'><input type='submit' value='Back' /></form><br /><br /><br />";
            echo "<form action='Articles/$artSlug' method='post'><textarea name='whyReported' cols='40' rows='10' required></textarea><br /><input type='hidden' name='reportId' value='$artId' /><input type='submit' value='Report' /></form><br /><br /><br />";
        }
    } else {
        $repstmt = $db->prepare("SELECT approved FROM articles WHERE id = ?");
        ...
        if ($reported == '2') {
            ...
            $ckLog = $db->prepare(
                    "SELECT COUNT(*) FROM articleLog WHERE articleId=? && visitingIP=?");
            $ckLog->execute(array(
                    $artId,
                    $visitingIP
            ));
            ...
            if ($ckLogRow[0] == 0) {
                $logstmt = $db->prepare(
                        "INSERT INTO articleLog VALUES" . "(NULL,?,?,?,'0','0')");
                ...
                $logstmt2 = $db->prepare(
                        "UPDATE articles SET totalViews = totalViews + 1 WHERE id = ?");
            ...
            if ($inReplyTo != '0') {
                ...
                $artLN = $grow['lastName'];
                echo "<div style='margin:20px 0px;'><a href='Articles/$inReplyTo'>This article is a reply to:<br />- $artTitle<br />\t<span style='font-size:.75em;'>" .
            ...
            if ($editedDate != '0' &&
                    date("M j, Y", $postedDate) != date("M j, Y", $editedDate)) {
                echo "Edited<br />";
            } else {
                echo "<br />";
            }
            ...
            if ($myId != '0') {
                $repstmt2 = $db->prepare(
                        "SELECT approved FROM articles WHERE id=?");
                ...
                $reported = $reprow2[0];
                if ($reported == '1') {
                    echo "<div style='float:right; font-size:0.75em;'><a href='Articles/reportArticle::$artId'>Report this article</a></div><br>";
                }
            }
            ...
            if (file_exists("userPics/$authorId/$pic1Name.$pic1Ext") ||
                ...
                if (file_exists("userPics/$authorId/$pic2Name.$pic2Ext")) {
                    list ($width, $height) = (getimagesize(
                            "userPics/$authorId/$pic2Name.$pic2Ext") != null) ? getimagesize(
                            "userPics/$authorId/$pic2Name.$pic2Ext") : null;
            ...
            if ($myId != '0') {
                echo "<a href='editArticle/inReplyTo::$artId' style='text-decoration:underline;'>Write a reply to this article.</a>";
            } else {
                echo "<a href='#top' style='text-decoration:underline;'>Sign in to reply to this article.</a>";
            }
            $rcount = $db->prepare(
                    "SELECT COUNT(*) FROM articles WHERE inReplyTo=?");
    ...
    if ($myZip != 0) {
        ...
        for ($i = 1; $i <= 3; $i ++) {
            ...
            $stmt6 = $db->prepare(
                    "SELECT t1.id FROM articles AS t1 INNER JOIN users AS t2 INNER JOIN reported AS t3 ON t1.authorId = t2.id && t1.id = t3.articleId${"where" .$i} && t3.reportedTime >= '1' && t3.clearedTime >= '1' && t1.catId = ? && t1.postedDate <= '$time' ORDER BY t1.postedDate DESC LIMIT 12");
            $stmt6->execute(array(
                    $catId
            ));
        ...
        $stmt18 = $db->prepare(
                "SELECT t1.id FROM articles AS t1 INNER JOIN reported AS t3 ON t1.id = t3.articleId WHERE t3.reportedTime >= '1' && t3.clearedTime >= '1' && t1.catId = ? && t1.postedDate <= '$time' ORDER BY t1.postedDate DESC LIMIT 36");
