...
if (isAdmin($myId)) {
    ...
    if (filter_input(INPUT_POST, 'createSlugs', FILTER_SANITIZE_NUMBER_INT) == 1) {
        $t = htmlspecialchars(trim($_POST["table"]));
        $f = htmlspecialchars(trim($_POST["field"]));
        $c = $db->prepare("SELECT id, $f FROM $t");
        $c->execute();
        while ($cr = $c->fetch()) {
            $id = $cr['id'];
            $slug = slugify($cr[$f]);
            $check = $db->prepare("SELECT id FROM $t WHERE slug = ?");
            $check->execute(array(
                    $slug
            ));
            $checkR = $check->fetch();
    ...
    while ($row1 = $stmt1->fetch()) {
        ...
        echo "</article>";
        ?>
					<form style="margin-top:30px;" action="Gadmin" method="post">
    ...
    while ($row = $substmt2->fetch()) {
        ...
        while ($prow2 = $pst2->fetch()) {
            ...
            $photoCaption = nl2br(
                    make_links_clickable($prow2['photoCaption'], $highlightColor));
    ...
    ?>) - Chapters to approve
			...
			<article id="w" style="display:none; padding:10px; margin:10px 0px; border:1px solid #aaaaaa;">
				<?php
    echo "<form action='$page' method='post'><table cellspacing='0' cellpadding='10'>";
    $getWr = $db->prepare(
    ...
    while ($row29 = $stmt29->fetch()) {
        ...
        echo "<tr>";
        echo "<td style='border:1px solid $highlightColor;'><form action='$page' method='post'><input type='text' name='title' value='$title' size='75' /><br />";
    ...
    ?>; padding:10px; margin:10px 0px; border:1px solid #aaaaaa;">
				<?php
    echo "<form action='Gadmin/showAds' method='post'>View advertising for:<br /><select name='getAdId' size='1'>\n";
    ...
    if (filter_input(INPUT_POST, 'getAdId', FILTER_SANITIZE_NUMBER_INT)) {
        ...
        echo "<form action='Gadmin' method='post' enctype='multipart/form-data'><tr><td style='border:1px solid $highlightColor; padding:5px;' colspan='3'>";
        ...
        while ($row = $stmt->fetch()) {
            ...
            echo "<table style='border:1px solid $highlightColor; width:100%;' cellspacing='5px'>\n";
            echo "<form action='Gadmin' method='post' enctype='multipart/form-data'><tr><td style='border:1px solid $highlightColor; padding:5px;' colspan='4'>";
    ...
    ?>
		...
		<section>
			...
			<article id="users" style="display:none; padding:10px; margin:10px 0px; border:1px solid #aaaaaa;">
				...
				<form action='Gadmin' method='post'><input type="submit" value=" Clear "><input type="hidden" name="clearUsers" value="1"></form>
			</article>
		</section>
		<section>
			...
			<article id="slugs" style="display:none; padding:10px; margin:10px 0px; border:1px solid #aaaaaa;">
				...
				<form action='Gadmin' method='post'>
				Table to create the slugs on: <input type="text" name="table" value="">
				The field to create the slug from: <input type="text" name="field" value="">
				<input type="submit" value=" Create Slugs ">
				<input type="hidden" name="createSlugs" value="1">
				</form>
			</article>
		</section>
	</main>
