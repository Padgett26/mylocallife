...
$questionNumber = (filter_input(INPUT_GET, 'questionNumber', FILTER_SANITIZE_NUMBER_INT)) ? filter_input(INPUT_GET, 'questionNumber', FILTER_SANITIZE_NUMBER_INT) : 0;
if ($questionNumber == 0) {
    $questionNumber = (filter_input(INPUT_POST, 'questionNumber', FILTER_SANITIZE_NUMBER_INT)) ? filter_input(INPUT_POST, 'questionNumber', FILTER_SANITIZE_NUMBER_INT) : 0;
}
$stmt = $db->prepare("SELECT COUNT(*) FROM survey WHERE id = ? && startDate <= ? && endDate >= ?");
$stmt->execute(array($surveyId, $time, $time));
$row = $stmt->fetch();
$sId = ($row[0] == 1) ? $surveyId : FALSE;

if ($sId) {
    if (filter_input(INPUT_COOKIE, 'responderId', FILTER_SANITIZE_NUMBER_INT) < 1 || !filter_input(INPUT_COOKIE, 'responderId', FILTER_SANITIZE_NUMBER_INT)) {
        setcookie("responderId", filter_var($visitingIP, FILTER_SANITIZE_NUMBER_INT), $time + 1209600, "/", "mylocal.life", 0);
    }
    ...
    foreach ($_POST as $k => $v) {
        if (preg_match("/^answer([1-9][0-9]*)$/", $k, $match)) {
            ...
            $acheck = $db->prepare("SELECT COUNT(*) FROM surveyAnswers WHERE responderId = ? && answerNum = ?");
            $acheck->execute(array($IP, $aNum));
            $achrow = $acheck->fetch();
            if ($achrow[0] == 1) {
                $aUp = $db->prepare("UPDATE surveyAnswers SET answer = ? WHERE responderId = ? && answerNum = ?");
                $aUp->execute(array($val, $IP, $aNum));
            } else {
                $aUp = $db->prepare("INSERT INTO surveyAnswers VALUES(NULL,?,?,?,?,'0','0')");
                $aUp->execute(array($sId, $IP, $time, $val, $aNum));
            }
        }
    }

    $answered = array();
    $s = $db->prepare("SELECT answer,answerNum FROM surveyAnswers WHERE surveyId=? && responderId = ?");
    $s->execute(array($sId, $IP));
    while ($sr = $s->fetch()) {
        $a = $sr['answer'];
        $an = $sr['answerNum'];
        if ($a != "" && $a != " ") {
    ...
    $s1->execute(array($sId));
    $s1r = $s1->fetch();
    $userId = $s1r['userId'];
    $surveyTitle = $s1r['surveyTitle'];
    $introText = nl2br(make_links_clickable(html_entity_decode($s1r['introText'], ENT_QUOTES), $highlightColor));
    $exitText = nl2br(make_links_clickable(html_entity_decode($s1r['exitText'], ENT_QUOTES), $highlightColor));
    $intropicName = $s1r['picName'];

    $qNum = array();
    $s2 = $db->prepare("SELECT questionNum FROM surveyQuestions WHERE surveyId=? ORDER BY questionNum");
    $s2->execute(array($sId));
    while ($s2r = $s2->fetch()) {
        $qn = $s2r['questionNum'];
        $qNum[] = $qn;
    }
    ...
    if ($questionNumber == 0) {
        ...
        $a = $db->prepare("SELECT answer FROM surveyAnswers WHERE responderId=? && surveyId = ? && answerNum = ?");
        $a->execute(array($IP, $sId, $questionNumber));
        $ar = $a->fetch();
        $answer = $ar['answer'];
        $a1 = $db->prepare("SELECT question,picName FROM surveyQuestions WHERE surveyId = ? && questionNum = ?");
        $a1->execute(array($sId, $questionNumber));
        $a1r = $a1->fetch();
    ...
    $s = $db->prepare("SELECT id,surveyTitle FROM survey WHERE startDate <= ? && endDate >= ? ORDER BY RAND()");
    $s->execute(array($time, $time));
    $b = 0;
    while ($srow = $s->fetch()) {
        $surveyId = $srow['id'];
