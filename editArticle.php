<?php
$inReplyTo = (filter_input(INPUT_GET, 'inReplyTo', FILTER_SANITIZE_NUMBER_INT)) ? filter_input(INPUT_GET, 'inReplyTo', FILTER_SANITIZE_NUMBER_INT) : '0';
if ($myAccess != 0) {
		...
		<div id='mainTableBox' style="padding:10px;">
			<form action="index.php?page=myAccount" method="post" enctype='multipart/form-data'>
				...
				if ($inReplyTo != '0') {
					...
					$artLN = $grow['lastName'];
					echo "<div style='margin:20px 0px;'>This article is a reply to:<br />- $artTitle<br />\t<span style='font-size:.75em;'>" . date("M j, Y", $artPosted) . "\t by $artFN $artLN</span></div>";
				}
				if ($editedDate > '1') {
					echo "Previous edit date: " . date("M j, Y", $editedDate) . "<br /><br />";
				}
				?>
				...
				To include a website link in your text, simply put the full address in your text (ex. <?php
																										...
																										echo make_links_clickable('https://mylocal.life/index.php', $highlightColor);
																										?> or <?php

				echo make_links_clickable('https://mylocal.life', $highlightColor);
