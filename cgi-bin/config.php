...
// *** Log out ***
if (! empty($GET['logout']) && htmlspecialchars(trim($_GET["logout"]), ENT_QUOTES) == 'yep') {
	destroySession();
...
if (filter_input(INPUT_POST, 'login', FILTER_SANITIZE_NUMBER_INT) == "1") {
	...
	if ($lrow1[0] < 3) {
		...
		if ($login2row['id']) {
			...
			setcookie("staySignedIn", $_SESSION['myId'], $time + 1209600, "/", "mylocal.life", 0); // set
			                                                                                       // for
			                                                                                       // 14
			                                                                                       // days
...
if ($myId == '0' && (empty($_GET["logout"]))) {
	$myId = (filter_input(INPUT_COOKIE, 'staySignedIn', FILTER_SANITIZE_NUMBER_INT) >= '1') ? filter_input(INPUT_COOKIE, 'staySignedIn', FILTER_SANITIZE_NUMBER_INT) : '0'; // are
	                                                                                                                                                                        // they
	                                                                                                                                                                        // logged
	                                                                                                                                                                        // in
}
...
if (filter_input(INPUT_GET, 'myZip', FILTER_SANITIZE_NUMBER_INT)) {
	setcookie("myZip", filter_input(INPUT_GET, 'myZip', FILTER_SANITIZE_NUMBER_INT), $time + 5184000, "/", "mylocal.life", 0); // set
	                                                                                                                           // for
	                                                                                                                           // 60
	                                                                                                                           // days
} else {
...
// if (empty($url_data[0])) {
$page = (isset($_GET["page"])) ? htmlspecialchars(trim($_GET["page"]), ENT_QUOTES) : "Home";
// } else {
// $page = $url_data[0];
// $slug = $url_data[1];
// }
...
if (! empty($_GET["articleCategory"])) {
	$articleCategory = htmlspecialchars(trim($_GET["articleCategory"]), ENT_QUOTES);
}
if (! empty($_GET["classifiedsCategory"])) {
	$classifiedsCategory = htmlspecialchars(trim($_GET["classifiedsCategory"]), ENT_QUOTES);
}
