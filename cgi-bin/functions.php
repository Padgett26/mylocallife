...
function sendVerificationEmail ($toId, $firstName, $email, $verifyCode)
{
    ...
    mail($email, 'Please verify your email address to access My Local Life',
            $message, $headers);
}
...
{
    $stmt = $db->prepare(
            "SELECT photoTitle,photoText,authorId FROM photoJournalism WHERE id = ?");
    $stmt->execute(array(
            $getId
    ));
    while ($row = $stmt->fetch()) {
        ...
        $authorId = $row['authorId'];
        $stmt2 = $db->prepare(
                "SELECT photoName,photoExt FROM photoList WHERE photoId = ? ORDER BY photoOrder LIMIT 1");
        $stmt2->execute(array(
                $getId
        ));
        $row2 = $stmt2->fetch();
...
{
    $stmt = $db->prepare(
            "SELECT userId, blogTitle, blogDesc, blogPic, blogPicExt FROM blogDescriptions WHERE id = ?");
    $stmt->execute(array(
            $getId
    ));
    while ($row = $stmt->fetch()) {
...
{
    $stmt = $db->prepare(
            "SELECT title FROM myWritings WHERE bookId = ? AND authorId = ?");
    $stmt->execute(array(
            $bookId,
            $authorId
    ));
...
{
    $stmt = $db->prepare(
            "SELECT userId, busiName, busiPhone, busiPic1, busiPicExt1 FROM busiListing WHERE id = ?");
    $stmt->execute(array(
            $getId
    ));
    while ($row = $stmt->fetch()) {
...
function displayClassified ($getId, $db, $highlightColor)
{
    ...
    echo "<article class='pageBoxesHalf' style='float:left; overflow:hidden; margin:10px; padding:20px; font-size:0.75em; text-align:center; border:0px solid $highlightColor; width:370px; box-shadow: 5px 5px 5px grey;'>\n";
    echo "<div style='font-size:0.75em; text-align:center;'>" . $category .
            "</div><br />";
...
function displayDirectory ($getId, $db, $highlightColor)
{
    $statement = $db->prepare(
            "SELECT userId, firstName, lastName, businessName, phone1, phone1Desc, phone2, phone2Desc, email, address1, address2 FROM directory WHERE showListing=? && id=? ORDER BY lastName, businessName");
...
function isArticleReported ($checkId)
{
    $stmt = db()->prepare("SELECT approved FROM articles WHERE id = ?");
    $stmt->execute(array(
            $checkId
    ));
    $row = $stmt->fetch();
...
function getReplies ($getId)
{
    $gr = db()->prepare(
            "SELECT t1.id, t1.articleTitle, t1.postedDate, t2.firstName, t2.lastName FROM articles AS t1 INNER JOIN users AS t2 ON t1.authorId = t2.id WHERE t1.inReplyTo = ? ORDER BY t1.postedDate");
    $gr->execute(array(
            $getId
    ));
    while ($grow = $gr->fetch()) {
        ...
        $artLN = $grow['lastName'];
        echo (isArticleReported($Aid)) ? "<div style='margin:10px 0px 0px 10px;'>- *REPORTED* $artTitle<br />\t<span style='font-size:.75em;'>" .
                date("M j, Y", $artPosted) . "\t by $artFN $artLN</span><br />" : "<div style='margin:10px 0px 0px 10px;'><a href='index.php?page=Articles&articleDetail=$Aid'>- $artTitle<br />\t<span style='font-size:.75em;'>" .
                date("M j, Y", $artPosted) .
                "\t by $artFN $artLN</span></a><br />";
        getReplies($Aid);
        echo "</div>";
    }
}
...
function cashOut ($userId, $db)
{
    ...
    $co2 = $db->prepare(
            "UPDATE HHUsers SET coinQty = ?, cashOut = ? WHERE userId = ?");
    $co2->execute(array(
            '0',
            $tot
    ));
