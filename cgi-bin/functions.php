...
{
    $stmt = $db->prepare(
            "SELECT COUNT(*) FROM reported WHERE reportedBy != '0' && articleId = ? && reportedTime >= '1' && clearedTime = '0'");
    $stmt->execute(array(
            $checkId
    ));
    $row = $stmt->fetch();
...
function getReplies ($getId, $db)
{
    ...
    while ($grow = $gr->fetch()) {
        ...
        $artLN = $grow['lastName'];
        echo (isArticleReported($Aid, $db)) ? "<div style='margin:10px 0px 0px 10px;'>- *REPORTED* $artTitle<br />\t<span style='font-size:.75em;'>" .
