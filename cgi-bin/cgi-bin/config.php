...
try {
	$db_ccdc = new PDO("mysql:host=$ccdchost; dbname=$ccdcname", "$ccdcuser", "$ccdcpass");
} catch (PDOException $e) {
...
$stmtc = $db->prepare("SELECT COUNT(*) FROM classifieds WHERE displayUntil >= ?");
$stmtc->execute(array(
	$time
));
$rowc = $stmtc->fetch();
...
$CST = $time - (6 * 60 * 60); // Setting the time zone to central standard time for the sake of the holidays display
$weekdays = array(
	"Monday",
	"Tuesday",
	"Wednesday",
...
if (htmlspecialchars(trim($_GET["logout"])) == 'yep') {
	destroySession();
...
if (filter_input(INPUT_POST, 'login', FILTER_SANITIZE_NUMBER_INT) == "1") {
	...
	$lstmt1 = $db->prepare("SELECT COUNT(*) FROM logInFails WHERE (userId = ? || ipAddress = ?) && tryTime >= ($time - 300)");
	...
	if ($lrow1[0] < 3) {
		...
		$login2stmt = $db->prepare("SELECT * FROM users WHERE email = ? AND password = ?");
		...
		if ($login2row['id']) {
			...
			setcookie("staySignedIn", $_SESSION['myId'], $time + 1209600, "/", "mylocal.life", 0); // set for 14 days
			$lstmt2 = $db->prepare("DELETE FROM logInFails WHERE userId = ? || ipAddress = ?");
			...
			$lstmt3 = $db->prepare("INSERT INTO logInFails VALUES(NULL,?,?,?,'0','0','0')");
...
} elseif (htmlspecialchars(trim($_COOKIE["myTheme"]))) {
...
// *** Delete a user ***
if (filter_input(INPUT_POST, 'deleteMyId', FILTER_SANITIZE_NUMBER_INT) == $myId && $myId != '0') {
...
if (filter_input(INPUT_GET, 'myZip', FILTER_SANITIZE_NUMBER_INT)) {
	setcookie("myZip", filter_input(INPUT_GET, 'myZip', FILTER_SANITIZE_NUMBER_INT), $time + 5184000, "/", "mylocal.life", 0); // set for 60 days
} else {
	if ($myId != '0') {
		...
		$row = $stmt->fetch();
		setcookie("myZip", $row['zip'], $time + 5184000, "/", "mylocal.life", 0); // set for 60 days
	}
}

$myZip = (filter_input(INPUT_COOKIE, 'myZip', FILTER_SANITIZE_NUMBER_INT)) ? filter_input(INPUT_COOKIE, 'myZip', FILTER_SANITIZE_NUMBER_INT) : $defaultZip;
...
if (htmlspecialchars(trim($_GET["articleCategory"]))) {
	$articleCategory = htmlspecialchars(trim($_GET["articleCategory"]));
}
if (htmlspecialchars(trim($_GET["classifiedsCategory"]))) {
...
if (filter_input(INPUT_POST, 'feedback', FILTER_SANITIZE_NUMBER_INT) && filter_input(INPUT_POST, 'rating', FILTER_SANITIZE_NUMBER_INT) != NULL) {
	...
	$feed = $db->prepare("INSERT INTO feedback VALUES(NULL, ?, ?, ?, '0',?,'0','0')");
	$feed->execute(array(
		$feedback,
		$rating,
		$feedText,
