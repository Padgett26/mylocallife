...
if ($photoId) {
    ...
    $logstmt = $db->prepare(
            "INSERT INTO photoLog VALUES" . "(NULL,?,?,'0','0','0')");
    $logstmt->execute(array(
            $photoId,
            $time
    ));
    $logstmt2 = $db->prepare(
            "UPDATE photoJournalism SET totalViews = totalViews + 1 WHERE id = ?");
    ...
    ?>
  <div style='float:right; margin:3px 10px 0px 0px;' class="fb-share-button" data-href="https://mylocal.life/index.php?page=Photo&photoShow=<?php

echo $photoId;
    ?>" data-layout="button" data-size="small"><a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=https%3A%2F%2Fmylocal.life%2Findex.php%3Fpage%3DPhoto%26photoShow%3D<?php

echo $photoId;
    ?>&amp;src=sdkpreparse" class="fb-xfbml-parse-ignore">Share</a></div>
  <div style='float:right; margin:3px 10px 0px 0px;'><a class="twitter-share-button" href="https://twitter.com/intent/tweet?original_referer=https%3A%2F%2Fmylocal.life%2Findex.php?page=Photo%26photoShow=<?php

echo $photoId;
    ?>&ref_src=twsrc%5Etfw&related=twitterapi%2Ctwitter&tw_p=tweetbutton&url=https%3A%2F%2Fmylocal.life%2Findex.php?page=Photo%26photoShow=<?php
...
echo $photoId;
  ...
  <?php
    ...
    $stmt18 = $db->prepare(
            "SELECT id FROM photoJournalism WHERE approved = ? ORDER BY postedDate DESC LIMIT 36");
