...
if ($myAccess == '3') {
	...
	if (filter_input(INPUT_POST, 'createSlugs', FILTER_SANITIZE_NUMBER_INT) == 1) {
		...
		while ($cr = $c->fetch()) {
			...
			$check->execute(array($slug));
			$checkR = $check->fetch();
			$count = $checkR[0];
			if ($count >= 1) {
				$slug .= "-" . $count;
			}
			$saveSlug = $db->prepare("UPDATE articles SET slug = ? WHERE id = ?");
			$saveSlug->execute(array($slug, $id));
		}
	}

	// Writing processing
	if (filter_input(INPUT_POST, 'writingUp', FILTER_SANITIZE_NUMBER_INT) >= 1) {
		$writingId = filter_input(INPUT_POST, 'writingUp', FILTER_SANITIZE_NUMBER_INT);
		$writingApprove = filter_input(INPUT_POST, 'writingApprove', FILTER_SANITIZE_NUMBER_INT);
...
?>
	<main id='mainTableBox' style="position:relative; top:0px; left:0px; padding:10px 0px 0px 10px;">
		<section>
			<?php
			$stmt0 = $db->prepare("SELECT COUNT(*) FROM reported WHERE reportedBy = ? && clearedTime = ?");
			$stmt0->execute(array(
				'0',
				'0'
			));
			...
			<article id="newArticles" style="display:none; padding:10px; margin:10px 0px; border:1px solid #aaaaaa;">
				<?php
				$stmt1 = $db->prepare("SELECT articleId, whyReported FROM reported WHERE reportedBy = ? && clearedTime = ? ORDER BY reportedTime");
				$stmt1->execute(array(
					'0',
					'0'
				));
		...
		<section>
			<?php
			$p1 = $db->prepare("SELECT COUNT(*) FROM photoJournalism WHERE approved = ?");
			$p1->execute(array(
				'0'
			));
			$pr1 = $p1->fetch();
		...
		<section>
			<?php
			$stmtw = $db->prepare("SELECT COUNT(*) FROM myWritings WHERE approved = '0'");
			$stmtw->execute();
			$roww = $stmtw->fetch();
			...
			<article id="w" style="display:none; padding:10px; margin:10px 0px; border:1px solid #aaaaaa;">
				...
				$getWr = $db->prepare("SELECT * FROM myWritings WHERE approved = '0' ORDER BY editDate");
				$getWr->execute();
				while ($getWR = $getWr->fetch()) {
					...
					$chText = $getWR['chText'];
					$getAuth = $db->prepare("SELECT firstName, lastName FROM users WHERE id = ?");
					...
					echo "<div style='cursor:pointer;' onclick='toggleview(\"writing" . $wId . "\")'>$title by $firstName $lastName Part $part Chapter $chapter</div>";
		...
		<section>
			<?php
			$stmt2 = $db->prepare("SELECT COUNT(*) FROM reported WHERE reportedBy != ? && clearedTime = ?");
			$stmt2->execute(array(
				'0',
				'0'
			));
			...
			<article id="reportedArticles" style="display:none; padding:10px; margin:10px 0px; border:1px solid #aaaaaa;">
				<?php
				$stmt3 = $db->prepare("SELECT id, articleId, whyReported FROM reported WHERE reportedBy != ? && clearedTime = ? ORDER BY reportedTime");
				$stmt3->execute(array(
					'0',
					'0'
				));
				while ($row3 = $stmt3->fetch()) {
					...
					$articleText = nl2br(make_links_clickable(html_entity_decode($subrow4['articleText'], ENT_QUOTES), $highlightColor));
					$catId = $subrow4['catId'];
					...
					$substmt5 = $db->prepare("SELECT category FROM articleCategories WHERE id=?");
					...
					$substmt6 = $db->prepare("SELECT firstName, lastName FROM users WHERE id=?");
		...
		<section>
			...
			<article id="fb" style="display:none; padding:10px; margin:10px 0px; border:1px solid #aaaaaa;">
				...
				while ($row6 = $stmt6->fetch()) {
					...
					$stmt7 = $db->prepare("SELECT firstName, lastName, email FROM users WHERE id=?");
					$stmt7->execute(array(
						$fbUserId
					));
					$row7 = $stmt7->fetch();
		...
		<section>
			...
			<article id="ads" style="display:<?php
				...
				$ads = $db->prepare("SELECT id, firstName, lastName, email FROM users ORDER BY lastName");
				$ads->execute();
				while ($adsrow = $ads->fetch()) {
					...
					if (filter_input(INPUT_POST, 'getAdId', FILTER_SANITIZE_NUMBER_INT) == $id) {
				...
				if (filter_input(INPUT_POST, 'getAdId', FILTER_SANITIZE_NUMBER_INT)) {
					...
					while ($row = $stmt->fetch()) {
						...
						if (file_exists("userPics/$userId/" . $adName . "." . $adExt)) {
							echo "<img src='userPics/$userId/" . $adName . "." . $adExt . "' alt='' style='max-height:200px; max-width:500px;' /><br />";
						}
						echo "Upload a new graphic:<br />For the top ad use a max size of 1100px X 100px.<br />";
						echo "For the side ads use a max size of 200px X 300px.<br /><br />";
						echo "<input type='file' name='adImage' /></td></tr>\n";
